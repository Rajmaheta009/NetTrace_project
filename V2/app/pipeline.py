...
touching the shared GraphStore and CaseStore, and the only place entity-id remapping happens
...
def run_import(request: ImportRequest) -> ImportResponse:
    ...
    # Case-aware ingestion: validate target case exists and is not closed/archived
    target_case_id = request.case_id or case_manager.active_case_id
    target_case = case_manager.get_case(target_case_id)
    if not target_case:
        # Fallback to active case
        target_case = case_manager.get_active_case()
        if not target_case:
            raise ImportValidationError(f"Target case '{target_case_id}' does not exist.")

    target_status = target_case.status.value if hasattr(target_case.status, "value") else str(target_case.status)
    if target_status.lower() in ("closed", "archived"):
        raise ImportValidationError(f"Target case '{target_case.case_id}' is {target_status} and cannot accept new imports.")
    ...
    ev = target_case.register_evidence(
    ...
    if request.type == "csv":
        ...
        if entity_rows:
            ...
            id_map = target_case.upsert_entities(entities)
            imported_entities += len(entities)

        if relationship_rows:
            # Build a raw-id -> global-id map from target case entities (plus fallback store)
            raw_to_global = {e.id: e.id for e in target_case.entities.values()}
            for e in target_case.entities.values():
            ...
            target_case.add_relationships(relationships)
        ...
        raw_to_global = {**id_map, **{e.id: e.id for e in target_case.entities.values()}}
        for e in target_case.entities.values():
            raw_to_global[e.name] = e.id
            raw_to_global[e.name.lower()] = e.id
            for a in e.aliases:
        ...
        for i, chunk in enumerate(chunks):
            ...
            for ent in chunk_entities:
                ent.evidence_id = ev.evidence_id
                ent.source_file = ev.filename
            id_map = target_case.upsert_entities(chunk_entities)
            ...
            for rel in chunk_relationships:
                ...
                if src in target_case.entities and tgt in target_case.entities:
            ...
            target_case.add_relationships(remapped)
...
def reset_and_load_sample() -> ImportResponse:
    ...
    r1 = run_import(_ImportRequest(type="csv", content=SAMPLE_ENTITIES_CSV, source_label="sample_entities.csv", case_id=active_case.case_id))
    r2 = run_import(_ImportRequest(type="csv", content=SAMPLE_RELATIONSHIPS_CSV, source_label="sample_relationships.csv", case_id=active_case.case_id))
            ...
            if fpath.exists() and fpath.stat().st_size > 0:
                try:
                    with open(fpath, "r", encoding="utf-8") as f:
                        content = f.read()
                    resp = run_import(_ImportRequest(content=content, source_label=fname, case_id=active_case.case_id))
